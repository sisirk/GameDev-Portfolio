<template>
  <div>
    ...
    >
      <div class="tech-lightbox-content" role="dialog" aria-modal="true">
...
export default Vue.extend({
  ...
  mounted() {
    // Delegated click handler: open lightbox when a tech-hover-card is clicked
    const onClick = (ev: Event) => {
      ...
      if (src) this.openLightbox(src, title);
    };
...
.tech-gallery {
  ...
  gap: 18px;
  max-width: 980px;        // keeps gallery from stretching too wide
  margin: 18px auto 0;
}
...
.tech-lightbox-title {
  font-weight: 700;
  color: #ffffff;
  margin: 4px 42px 10px 6px; // space for the close button
  text-align: left;
}
